...
class AtomGraphDataset:  # TODO: This class is cursed! please don't use it ;)
    ...
    def augment(self, dataset, validator: Optional[Callable] = None):
        ...
        if not validator(self, dataset):
            raise ValueError('Cutoff not consistent')
    ...
    def assign_datawise_weight(self, weight_dict, label=None):
        if label is None:
            data_list = self.to_list()
        else:
            data_list = self.dataset[label]
        for datum in data_list:
            datum[KEY.DATA_WEIGHT] = weight_dict
