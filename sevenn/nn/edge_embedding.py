...
class BesselBasis(nn.Module):
    ...
    def __init__(
        ...
        prefactor = None
        if normalize == 'nequip':
            prefactor = 2.0 / cutoff_length
        elif normalize == 'ortho':
            prefactor = (2.0 / cutoff_length) ** 0.5
        else:
            raise ValueError(f'Unknown Bessel basis normalize: {normalize}')
        self.prefactor = prefactor

        pi_c = math.pi / cutoff_length
        coeffs = torch.FloatTensor(
            [n * pi_c for n in range(1, bessel_basis_num + 1)]
        )
        if trainable_coeff:
            self.coeffs = nn.Parameter(coeffs)
        else:
            self.register_buffer('coeffs', torch.FloatTensor(coeffs))
