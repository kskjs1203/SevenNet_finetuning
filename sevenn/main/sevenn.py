...
fisher_help = 'compute fisher information from train set and model'
...
def main(args=None):
    ...
    """
    input_yaml, working_dir, screen, distributed, fisher = cmd_parse_main(args)
    ...
    if fisher or train_config[KEY.CONTINUE][KEY.CALC_FISHER]:
        Logger().writeline(
            'Compute Fisher information from model and training set.'
            ' No actual training will be performed.'
            ' Ignore reheasal if it was True'
        )
        train_config[KEY.CONTINUE][KEY.CALC_FISHER] = True
        data_config[KEY.REHEARSAL] = False
    ...
    # run train
    if global_config[KEY.REHEARSAL]:
...
def cmd_parse_main(args=None):
    ...
    ag.add_argument(
        'input_yaml', help=input_yaml_help, type=str
    )
    ag.add_argument(
        '-w',
        '--working_dir',
    ...
    ag.add_argument(
        '-fs', '--fisher', help=fisher_help, action='store_true'
    )
    ag.add_argument(
        '-s', '--screen', help=screen_help, action='store_true'
    )
    ag.add_argument(
    ...
    wd = args.working_dir
    return input_yaml, wd, args.screen, args.distributed, args.fisher
